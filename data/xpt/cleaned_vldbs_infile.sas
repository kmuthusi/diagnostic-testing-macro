...
/********************************************************
** program: C:\PhD\Diagnostic testing\SAS\data\xpt\cleaned_vldbs_infiles.sas  
** programmer: savasas  
** date: 21 Oct 2021 
** comments: SAS program to read and label:  
**  C:\PhD\Diagnostic testing\SAS\data\xpt\cleane.xpt 
 ...
 ** this version of _infile_report.do will be overwritten if all goes well. **; 
 data _null_;
file "C:\PhD\Diagnostic testing\SAS\data\xpt\cleaned_vldbs_infile_report.do"; 
 put "capture program drop sas_rep"; 
 put "program define sas_rep, rclass"; 
 put "di as err "" SAS failed to create cleaned_vldbs "" "; 
 put "di as err "" Look at {view C:\PhD\Diagnostic testing\SAS\data\xpt\cleaned_vldbs_infile.log:C:\PhD\Diagnostic testing\SAS\data\xpt\cleaned_vldbs_infile.log} to see what error occurred. "" "; 
 put "local sas_rep_error= 1 "; 
 put "return local sas_rep_error ""`sas_rep_error\'"" "; 
 put "end"; 

libname library v9 "C:\PhD\Diagnostic testing\SAS\data\xpt\" ;  

options fmtsearch=(out.cleaned_vldbs);  

libname out v9 "C:\PhD\Diagnostic testing\SAS\data\xpt\"  ;  

libname raw xport "C:\PhD\Diagnostic testing\SAS\data\xpt\cleane.xpt";  


options NoQuoteLenMax;
data formats;
length fmtname $32 start end 8 label $32000;
infile "C:\PhD\Diagnostic testing\SAS\data\xpt\cleaned_vldbs_formats.txt" lrecl=32075 truncover ; 
input fmtname 1-32 start 34-53 end 55-74 label 76-32000;
run; 


proc format library= library.cleaned_vldbs cntlin= work.formats(where= (fmtname ^= ""));
run; quit;

%macro redo;
 ** Check for this error message:
  * upcase(error:) File LIBRARY.cleaned_vldbs.CATALOG was created for a different operating system. **;
 %if &syserr.= 3000 %then %do;
   proc datasets library= library 
                 memtype= catalog 
                 nodetails nolist nowarn;
     delete cleaned_vldbs;
   run;
   ** now try it! **;
   proc format library= library.cleaned_vldbs cntlin= work.formats(where= (fmtname ^= ""));
   run; quit;
...
data out.cleaned_vldbs  (label= "-savasas- created dataset on %sysfunc(date(), date9.)"  rename=(
 ...
 FORM2COD= form2code FORM3PAT= form3patientid INFORM1= inform1 INFORM2= inform2 INFORM3= inform3
 ...
 DAPSONE 3 COTRIMOX 3 CONTRACE 3 MULTIVIT 3 OTHERARV $8 OTHERAR1 $10 OTHER 3 FORM2COD 3
...
format  SEX sexf. RECURREN yesnof. RECURRE1 yesnof. DECLINEG yesnof. FAILUREN yesnof.
 ...
 ASSESSME yesnof. OPTIMIZI yesnof. FORM2COD codef. PROV_COD prov_codef. M_DBS_AC yesnof.
...
proc printto print= "C:\PhD\Diagnostic testing\SAS\data\xpt\cleaned_vldbs_SAScheck.lst" new; 

 title "data= C:\PhD\Diagnostic testing\SAS\data\xpt\cleaned_vldbs: Compare results with Stata output."; 

 proc means    data= out.cleaned_vldbs; run;

 proc contents data= out.cleaned_vldbs; run;

 proc print    data= out.cleaned_vldbs (obs=5); run; 
